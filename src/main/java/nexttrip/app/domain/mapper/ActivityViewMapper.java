package nexttrip.app.domain.mapper;

import nexttrip.app.domain.dto.ActivtityViewDto;
import nexttrip.app.domain.dto.ItineraryViewDto;
import nexttrip.app.domain.entity.Activity;
import nexttrip.app.domain.entity.Itinerary;
import nexttrip.fw.mapper.EntityMapper;
import org.mapstruct.Mapper;
import org.mapstruct.factory.Mappers;

import java.util.List;

@Mapper(componentModel = "spring")
public interface ActivityViewMapper extends EntityMapper<ActivtityViewDto, Activity> {
    ActivtityViewDto toDto (Activity activity);
    List<ActivtityViewDto> toDto(List<Activity> activities);
}
